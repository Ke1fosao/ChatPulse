...
def split_owner_routes() -> None:
    ...
    groups: dict[str, list[tuple[str, str]]] = {
        "session": [], "overview": [], "users": [], "groups": [], "payments": [], "audit": []
    }
    ...
    for category, blocks in groups.items():
        ...
        write(f"app/api/owner/{category}.py", helper_import + "\n\n".join(block for _, block in blocks))
    ...
    for category, names in module_names.items():
        route_lines.append(f"from app.api.owner.{category} import {', '.join(names)}  # noqa: E402,F401")
...
def split_user_control_repository() -> None:
    ...
    repository_class = next(
        node for node in tree.body if isinstance(node, ast.ClassDef) and node.name == "UserControlRepository"
    )
    ...
    base_method_names = [
        name for name in methods if name == "__init__" or name == "resolve_actor" or name.startswith("_")
    ]
    write(
        "app/repositories/user_control/base.py",
        prefix + "class UserControlBase:\n" + "\n\n".join(methods[name] for name in base_method_names),
    )
    public = [name for name in methods if name not in base_method_names]
    categories: dict[str, list[str]] = {
        "queries": [], "restrictions": [], "staff": [], "notes_tags": [], "xp": [], "messaging": [], "audit": []
    }
    restriction_names = {"is_blocked", "get_block_info", "record_blocked_access", "block_user", "unblock_user"}
    for name in public:
    ...
    repository_imports.extend(["", "", f"class UserControlRepository({inheritance}):", "    pass", ""])
...
def move_feature_styles() -> None:
    ...
    main = main.replace('import { RootRouter } from "./routing/RootRouter";\n', 'import { RootRouter } from "./routing/RootRouter";\nimport "./styles/global.css";\n')
    main_path.write_text(main, encoding="utf-8")
    write("miniapp/src/styles/app.ts", "\n".join(f'import "./{name}.css";' for name in ["bottom-nav-v2", "blocked-account", "group-settings", "groups-v2", "group-center", "achievement-celebration", "achievement-collection", "achievement-card-fixes", "achievement-showcase-v3", "onboarding", "profile-experience", "year-summary"]))
    write("miniapp/src/owner/styles.ts", 'import "../styles/owner.css";\nimport "../styles/owner-revenue.css";\nimport "../styles/owner-mobile.css";\nimport "../styles/owner-user-control.css";')
    write("miniapp/src/premium/styles.ts", 'import "../styles/featured-premium.css";\nimport "../styles/premium.css";\nimport "../styles/premium-identity.css";\nimport "../styles/premium-purchase.css";')
    ...
    if 'import "./styles/app";' not in app:
        app = app.replace('import { isTelegramContext } from "./telegram/sdk";\n', 'import { isTelegramContext } from "./telegram/sdk";\nimport "./styles/app";\n')
    app_path.write_text(app, encoding="utf-8")
    for rel, style_import in [("miniapp/src/premium/PremiumContext.tsx", 'import "./styles";\n'), ("miniapp/src/vip/VipApp.tsx", 'import "./styles";\n')]:
        p = ROOT / rel
