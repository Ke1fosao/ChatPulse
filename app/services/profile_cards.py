...
def _rounded_metric(
    ...
    draw.text(
        (left + 84, top + 21),
        label.upper(),
        font=_font(FONT_BOLD_PATHS, 22),
        fill=(154, 161, 184),
    )
    draw.text(
        (left + 24, top + 82),
        value,
        font=_font(FONT_BOLD_PATHS, 46),
        fill=(250, 250, 255),
    )
    draw.text(
        (left + 24, bottom - 45),
        caption,
        font=_font(FONT_REGULAR_PATHS, 22),
        fill=(139, 147, 171),
    )
...
def render_profile_card(payload: dict[str, Any]) -> bytes:
    ...
    draw.text(
        (246 - (level_box[2] - level_box[0]) / 2, 498),
        level,
        font=level_font,
        fill=(255, 255, 255),
    )
    ...
    metrics = (
        ...
        (
            "СЕРІЯ",
            str(int(quick.get("current_streak", 0))),
            "днів поспіль",
            (55, 42, 86),
        ),
        (
            "ПОВІДОМЛЕННЯ",
            str(int(quick.get("messages_7d", 0))),
            "за останні 7 днів",
            (35, 65, 82),
        ),
        (
            "ЗАХИСТ",
            str(int(quick.get("protection_left", 0))),
            "днів цього місяця",
            (31, 69, 69),
        ),
    )
    ...
    draw.text(
        (72, 1112),
        "Privacy-first Telegram analytics",
        font=small_font,
        fill=(129, 137, 160),
    )
