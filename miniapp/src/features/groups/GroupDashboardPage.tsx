  ...
  return (
    <div className="page group-dashboard">
      ...
      <div className="segmented-control" aria-label="Період статистики">
        {periods.map((period) => (
          <button
            className={data.period === period.id ? "is-active" : ""}
            key={period.id}
            type="button"
      ...
      <section className="panel personal-progress">
        ...
        <div className="personal-progress__badges">
          ...
          <span><Trophy size={16} /> рекорд {data.personal_progress.longest_streak}</span>
          <span><Shield size={16} /> захист {data.personal_progress.protection_left}</span>
        </div>
      </section>
      ...
      <section className="section-block">
        ...
        <div className="nomination-grid">
          {data.nominations.map((nomination) => (
            <article className="nomination-card" key={nomination.metric}>
              <span>{nomination.title.split(" ")[0]}</span>
              <strong>{nomination.display_name}</strong>
              <small>{nomination.value.toLocaleString("uk-UA")}</small>
            </article>
          ))}
        </div>
      </section>
